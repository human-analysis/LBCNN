-- mnist.lua
-- Adapted from Clement Farabet's implementation
...
local opt = lapp[[
   ...
   -m,--model         (default "cnn")       type of model tor train: cnn | lbcnn
   ...
   -k,--sparsity      (default 0.25)        sparsity for LBC unit
]]
...
if opt.network == '' then
   ...
   elseif opt.model == 'lbcnn' then
      ------------------------------------------------------------
      -- local binary convolutional network
      ------------------------------------------------------------
      -- stage 1 : mean suppresion -> filter bank -> squashing -> max pooling
      model:add(nn.RandomBinaryConvolution(01, 32, 5, 5, 1, 1, 0, 0))
      model:add(nn.Tanh())
      model:add(nn.SpatialConvolutionMM(32, 32, 1, 1))
      model:add(nn.SpatialMaxPooling(3, 3, 3, 3, 1, 1))
      -- stage 2 : mean suppresion -> filter bank -> squashing -> max pooling
      model:add(nn.RandomBinaryConvolution(32, 64, 5, 5, 1, 1, 0, 0))
      model:add(nn.Tanh())
